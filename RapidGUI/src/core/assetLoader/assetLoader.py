...
#########################################################################################
    ...
    def readRootAssets(self):
        rootAssetDictionary = {}
        rootAssetDictionary['window'] =  pd.read_csv('RapidGui\\assets\\'+ self.assetFile + '\\root\\window\\window.csv')
...
#########################################################################################
    ...
    def readTkInterUIAssets(self):
        ...
        tkInterUIAssetDictionary['button'] = pd.read_csv('RapidGui\\assets\\ui\\tkInterElements\\buttonUI.csv')
        tkInterUIAssetDictionary['label'] = pd.read_csv('RapidGui\\assets\\ui\\tkInterElements\\labelUI.csv')
        tkInterUIAssetDictionary['textBox'] = pd.read_csv('RapidGui\\assets\\ui\\tkInterElements\\textBoxUI.csv')
...
#########################################################################################
    ...
    def readLayoutAssets(self):
        ...
        path = "RapidGui\\assets\\" + self.assetFile + "\\layout"
        numScreens =  len(os.listdir(path))
        for screen in range(numScreens):
            screenAssetDictionary = {}
            fileName = 'RapidGui\\assets\\' + self.assetFile + '\layout' + '\screen' + str(screen)+ 'Elements.xlsx'
